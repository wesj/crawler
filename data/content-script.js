                    ...
                    if (request.readyState === 4) {
                        try {
                            ...
                            data.push({
                                ...
                                words: getWords(win),
                                headers: headers,
                                frames: Array.prototype.map.call(win.document.querySelectorAll("iframe"), function(frame) { return frame.src })
                            });
                        } catch(ex) { }

                        Promise.all(Array.prototype.map.call(win.frames, function(frame) {
                            return getDataForWindow(frame.contentWindow);
                        })).then(resolve);
                }
        };
...
getDataForWindow(window).then(function(res) {
    console.log("Emit", document.location.href, data);
