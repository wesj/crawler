    ...
    function getAnchors(win) {
        var anchors = win.document.querySelectorAll("a");
        var res = [];
    ...
    function getWords(win) {
        var scripts = win.document.body.querySelectorAll("script");
        Array.prototype.forEach.call(scripts, function(script) {
            script.parentNode.removeChild(script);
        })

        var styles = win.document.body.querySelectorAll("style");
        Array.prototype.forEach.call(styles, function(style) {
            style.parentNode.removeChild(style);
        });

        var allNodes = win.document.body.querySelectorAll("*");
    ...
    function getMetaTags(win) {
        var results = {};
        var meta = win.document.querySelectorAll("meta");
    ...
    function getLinks(win) {
        var results = {};
        var links = win.document.querySelectorAll("link");
    ...
    var ops = "[\/*\s+\(\.\-]"
    function getScriptInfo(win) {
        var scripts = win.document.scripts;
        var results = {};
        Array.prototype.forEach.call(scripts, function(script) {
            var url = script.url || win.document.location.href;
    ...
    function getDOMInfo(win) {
        var nodes = win.document.querySelectorAll("*");
        var result = {
    ...
    function getStyleInfo(win) {
        var sheets = win.document.styleSheets;
        var results = { };
        Array.prototype.forEach.call(sheets, function(sheet) {
            var url = sheet.href || win.document.location.href;
            if (!results[url]) results[url] = {
                media: "",
                selectors:  {},
                properties: {},
    ...
    if (document.contentType == "text/css") {
        ...
        request.onreadystatechange = function () {
            if (request.readyState === 4) {
                ...
                self.port.emit("html", [{
                    ...
                    styles: getStyleInfo(window),
                    scripts: {},
                    words: {},
                }]);
            }
        }
        request.open('HEAD', window.document.location, true);
        request.send(null);
        ...
        var data = [];
        function getDataForWindow(win) {
            return new Promise(function(resolve, reject) {
                try {
                    console.log("Get", win.document.location.href);
                    var request = new XMLHttpRequest();
                    request.onreadystatechange = function () {
                        console.log("Got", win.document.location.href, request.readyState);
                        if (request.readyState === 4) {
                            try {
                                var headers = {};
                                request.getAllResponseHeaders().split("\r\n").map(function(header) { return header.split(":") })
                                            .forEach(function(h) { headers[h[0]] = h[1]; });
                                ...
                                data.push({
                                    ...
                                    docCharset: win.document.characterSet,
                                    mimeType: win.document.contentType,
                                    anchors: getAnchors(win),
                                    images: getImages(win),
                                    meta: getMetaTags(win),
                                    links: getLinks(win),
                                    dom: getDOMInfo(win),
                                    styles: getStyleInfo(win),
                                    scripts: getScriptInfo(win),
                                    words: getWords(win),
                                    headers: headers,
                                    frames: Array.prototype.map.call(iframes, function(frame) { return frame.src })
                                });
                            } catch(ex) {
                                console.log("Error in request handler", ex);
                            }

                            try {
                                console.log("Get all", iframes);
                                var promises = [];
                                for (var i = 0; i < iframes.length; i++) {
                                    var frame = iframes[i];
                                    if (frame) {
                                        console.log("Done2", frame.src, frame.contentWindow);;
                                        promises.push(getDataForWindow(frame.contentWindow));
                                    }
                                }
                                ...
                                console.log("error getting iframes", ex);
                                resolve();
                            }
                        }
                    };
                    ...
                    request.onerror = function() {
                        console.log("Error getting", win.document.location);
                        resolve();
                    }
                    request.open('HEAD', win.document.location, true);
                    request.send(null);
                } catch(ex) {
        ...
        getDataForWindow(window).then(function(res) {
            self.port.emit("html", data);
        }).catch(function(ex) {
